...
size: buffer size
type:H265_VPS/H265_SPS/H265_PPS
*/
int h265_configure_parse(h265_stream_t* h, uint8_t* buffer, int size, int type)
{
	int nal_size, rbsp_size, rc;
	rbsp_size = nal_size = size;
	uint8_t* rbsp_buf = (uint8_t*)malloc(rbsp_size);
	rc = nal_to_rbsp(2, buffer, &nal_size, rbsp_buf, &rbsp_size);
